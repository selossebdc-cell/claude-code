# Face Soul Yoga — Memoire Projet

## Identite
- **Entreprise** : Face Soul Yoga
- **Dirigeante** : Aurelia
- **Secteur** : Bien-etre / Yoga / Formation en ligne (BtoC programmes transformationnels + BtoB formation Master The Method)
- **Ville** : Paris / Biarritz (double base)
- **Taille** : TPE (Aurelia + 1-3 personnes dont Laurie en support operationnel)
- **Programme** : Clarte & Autonomie — 6 mois, 19 seances
- **Periode** : fevrier 2026 → aout 2026 (dates exactes a confirmer)
- **Sessions** : 5 / 19 utilisees (prochaine session a caler cette semaine)
- **Tutoiement / Vouvoiement** : Tutoiement (proposition redigee en tutoiement)

## Notion IDs
- Dashboard : `304c3a2f-4255-80da-b8b2-fef4e17f7243`
- Meeting Agendas (data source) : `304c3a2f-4255-815e-b1b9-000b7082a58d`
- Objectifs & Actions (data source) : `304c3a2f-4255-81c3-82a0-000b8faa3263` (ID API : `304c3a2f-4255-8105-939a-cc224a563a66`)
- Fiche client (Mes Clients) : `304c3a2f-4255-810c-a392-e301c0c5603b`
- Feuille de Route (data source) : `304c3a2f-4255-8126-a334-000bb2fe5505`
- SOP & Process (data source) : `304c3a2f-4255-81ae-8bae-000bef8717d0`
- Page Feuille de Route Strategique : `304c3a2f-4255-814d-a857-d766711d8825`
- Contrat & Conditions : `304c3a2f-4255-810e-bb3e-c5f47138f131`
- Factures & Paiements : `304c3a2f-4255-816c-adf9-e638f6711ea5`
- Tracker de Temps (data source) : `304c3a2f-4255-81d2-b34e-000bc15dc1ac`
- Bibliotheque des Formations : `17a4075f-1431-4091-b1e1-2aa9076de222`

## Profil & Particularites
- **Personnalite** : Passionnee, exigeante, en quete de profondeur et d'authenticite. Tendance a l'eparpillement et a l'epuisement.
- **Style de communication** : Direct, emotionnel, investie. Parle avec passion de sa methode.
- **Neuro-diversite** : (non mentionne)
- **Niveau digital** : Intermediaire — utilise Notion, ManyChat, Brevo, CapCut, Canva mais outils sous-utilises (20% de potentiel), architecture fragmentee
- **Outils actuels** : Circle (remplace Kajabi+Uscreen — décidé session 4, Business 199€/mois), Bunny.net (stockage vidéo CDN), Brevo (email marketing — maintenu, 13K contacts), Wix (site — à refondre éducatif), ManyChat, CapCut, Canva, Google Drive, WhatsApp, Claude AI (adopté par Aurélia, connecteurs Canva/Drive/Calendar)
- **Outils en sortie** : Uscreen (migration vers Circle avant juillet 2026, coûte 7090€/an), Kajabi (migration vers Circle, coexistence temporaire)
- **VA** : Anam (Pakistan, Upwork, 500$) — migration vidéos Uscreen → Bunny.net + import Circle. Laurie supervise.
- **Ce qui la motive** : Devenir une vraie chef d'entreprise avec recul et fluidite, atteindre 1M EUR, transmettre sa methode, retrouver du temps pour creer
- **Ce qui la bloque** : Eparpillement, syndrome de l'imposteur, 80% du temps en operationnel, process manquants, peur de deleguer (budget + confiance), pas de vision claire

## Contrat signe
- **Programme** : Clarte & Autonomie — 6 mois
- **Investissement** : 8 000 EUR TTC (tarif preferentiel -20%)
- **Paiement** : 2 500 EUR a la signature + 5 500 EUR en echeancier (OPCO possible)
- **Seances** : 19 individuelles (1 audit 1h30 + 12 hebdo 1h + 6 bi-mensuelles 1h)
- **Inclus** : 1 agent IA sur-mesure (valeur 1 500 EUR), support WhatsApp, deplacement site
- **5 piliers contractuels** : Clarte & Vision, Parcours Client & Organisation, Automatisation & Outils, Systemes & Delegation, Pilotage & Perennisation
- **Objectifs contractuels** : Liberer 10-15h/sem, reprendre controle outils, structure pour 10K EUR/mois recurrents

## Audit Decouverte
> Informations extraites du CR de lancement et de la proposition commerciale

### Problematiques identifiees
1. Architecture technique fragmentee (Uscreen, Wix, Planity) — hemorragie financiere et mentale
2. Saturation operationnelle — 80% du temps en ops, gestion en mode "pompier"
3. Aucun process documente (score 2/5) — tout repose sur Aurelia
4. Outils sous-utilises a 20% de leur potentiel, abonnements en doublon
5. Dependance totale aux RS pour generer des ventes — besoin de "constamment poster"
6. Delegation impossible faute de process, budget et confiance

### Objectifs exprimes
1. Structurer les process et sortir du funnel de vente
2. Deleguer les taches sans valeur ajoutee
3. Vendre massivement les programmes signatures BtoC (299 EUR)
4. Repositionner vers l'expertise et la profondeur (Premium)
5. Construire un ecosysteme BtoC/BtoB capable de generer 1M EUR annuel

### Points de douleur
- "Plus de bande passante" — saturation totale
- "Je m'eparpille, je m'epuise sur des choses pas necessaires"
- Syndrome de l'imposteur — doute sur la legitimite
- Pas de vision claire, pas de points precis
- Onboarding client flou, mailing desorganise
- Couts techniques non maitrises

### Motivation principale
- "Je veux etre une vraie chef d'entreprise qui a du recul, anticipe et pilote sa boite sans la tete dans le guidon mais avec fluidite et joie"

## Questionnaire Fillout
> Reponses recues le 16 fevrier 2026 (commencees le 12, finalisees le 16)

### 1. Vision
- Mission : Developper et diffuser une methode de transformation globale (Face-Fascia-Posture-Mindset) pour les femmes
- BtoC : programmes transformationnels a forte valeur percue
- BtoB : certification Master The Method
- Vision 3 ans : Reference premium en Europe, 1M EUR annuel, ecosysteme hybride BtoC/BtoB, equipe legere
- Valeurs : Transmission, integrite, souverainete, authenticite

### 2. Strategie
- Score structuration : 5/10
- Organisation : Reactive et flexible
- Priorites 12 mois : Structurer process, sortir funnel de vente, reduire couts, deleguer, vendre programmes signatures BtoC (299 EUR), repositionner vers expertise

### 3. Parcours client
- Connaissance parcours : Partielle
- Perte prospects : "Je ne sais pas" (red flag — pas de visibilite)
- Automatisation : 19%
- Retours clients : Systematiques apres chaque projet
- Friction principale : Onboarding flou, mailing desorganise, gestion questions/reponses manuelle

### 4. Productivite
- Heures/semaine : 50h
- Repartition : 40h operationnel (80%) / 5h strategie (10%) / 5h gestion (10%)
- Taches chronophages : emails, devis, echanges informels marques/clients, Instagram, poster, monter, relances, planning
- Absence 2 semaines : "Tout continuerait mais avec tension"
- Documentation process : 2/5

### 5. Problematiques
- Plus gros blocage : "Mon temps, je m'eparpille, je m'epuise sur des choses pas necessaires et je pense que je n'ai pas de vision claire, de points precis, ou je veux etre vraiment"
- Frein strategie : "Trop d'operationnel et plus de bande passante"
- Baguette magique : "Les process et avoir une vraie clarte sur ou j'en suis"

### 6. Objectifs
- Posture : Plutot dirigeant mais trop dans l'operationnel
- Legitimite : Non — syndrome de l'imposteur
- Priorites classees : 1. Clarte & Audit / 2. Pilotage strategique / 3. Structuration parcours client / 4. Mindset & posture dirigeant / 5. Automatisation & delegation
- Pret au changement : 10/10
- Heures dispo accompagnement : 3h/semaine
- Attente coach : "La structure, mise en place de process, aider a la vision claire et m'aider a ne pas douter de ma legitimite"
- Objectif 6 mois : "Je veux etre une vraie chef d'entreprise qui a du recul, anticipe et pilote sa boite sans la tete dans le guidon mais avec fluidite et joie"

### 7. Revenus
- CA actuel : ~150k EUR, croissance +40%
- Ambition : 400-500k EUR (court terme), 1M EUR (3 ans)
- Objectif recurrent : 10K EUR/mois minimum
- Modele actuel : 100% abonnement → transition vers Produits secs + Upsell

## Patterns Observes

### Ce qui marche avec ce client
- Marque forte, capacite de production de contenu elevee
- Tresorerie saine malgre architecture couteuse
- Communaute engagee (~300 sur WhatsApp, les clientes se repondent entre elles)
- Retours clients systematiques (bonne relation client) — temoignages puissants
- Prete au changement a 100% (score 10/10)
- Laurie en support operationnel (a faire monter en puissance)
- Delegation contenu video en cours (monteur recrute, 45 reels/3 mois)
- Antoine recrute pour gerer les reels et social media — delegation contenu effective
- Capacite de production commerciale elevee (12K EUR en 1 mois avec 2 prestas)
- Laurie alignee sur le repositionnement premium
- Aurelia adopte Claude AI en autonomie (migration depuis ChatGPT)
- Vision se clarifie : FSY = rente automatisee, Aurelia Del Sol = premium
- Pricing stabilise session 3 : formation 1 950 EUR (early bird 1 550 EUR), abo 17 EUR/mois (139 EUR/an), accompagnement 399 EUR
- Evergreen formation : lancement realiste mai 2026, 10 inscriptions/mois, fin des cohortes
- Accompagnement sommeil : lancement juin 2026, 399 EUR, 18 videos
- YouScreen garde 1 an de plus (migration trop couteuse maintenant)
- Revenue projection realiste : 634K EUR/an (120 formations + 1 000 accompagnements)
- Contrats actuellement manuels (e-signature bulk) — automatisation Kajabi a explorer
- Laurie role de garde-fou strategique — freine Aurelia quand elle s'eparpille

### Blocages recurrents
- Eparpillement — "je pars dans tous les sens"
- Syndrome de l'imposteur — doute sur legitimite et prix (200-250 EUR/h en presentiel mais 299 EUR pour 28j en ligne)
- Dependance aux RS (poster = vendre)
- Pas de process → delegation impossible
- Incapacite a deconnecter — "j'ai du mal a me dire je vais partir une semaine et c'est OK"
- WhatsApp non cadre — clientes qui ecrivent le week-end sans limites, "les gens me prennent pour l'epicerie"
- Fatigue decisionnelle — "mon esprit est un peu fatigue" (session 2)
- Perte calendrier integre et app mobile = gros changement pour les membres (migration)

### Themes qui reviennent
- Process et clarte (sa priorite n°1)
- Migration technique (Kajabi + Bunny.net confirme — Circle elimine, Uscreen en sortie)
- Delegation a Laurie (montee en puissance, prend le lead sur migration)
- Separation FSY (mass-market) vs Aurelia Del Sol (premium) — deux marques, deux offres
- Posture CEO vs operationnel
- Legitimite des prix (transition petit prix → Premium)
- Gestion Paris/Biarritz necessite outils cloud flexibles
- Structure de l'offre B2C (base + upsells)
- Evergreen vs cohortes pour la formation B2B
- Objectif CA ambitieux (500K EUR 2026, 1M EUR a 3 ans)

### Citations cles
- "Je m'eparpille, je m'epuise sur des choses pas necessaires" (Fillout)
- "Plus de bande passante" (audit + Fillout)
- "Je veux etre une vraie chef d'entreprise qui pilote sa boite avec fluidite et joie" (Fillout)
- "Les process et avoir une vraie clarte sur ou j'en suis" (Fillout, baguette magique)
- "Il faut qu'on sorte du truc de 'non mais on fait du bien, namaste'. Ca fait 6-7 ans que je fais ca, je me suis formee, j'ai investi sur moi" (Session 1 — prise de conscience valeur)
- "2026, on arrete de travailler dans l'urgence" (Session 1 — engagement commun Aurelia + Laurie)
- "J'ai du mal a me dire je vais partir une semaine et c'est OK" (Session 1 — charge mentale)
- "Les gens me prennent pour l'epicerie" (Session 2 — absence de cadre devalue l'expertise)
- "Si on arrive a automatiser un maximum Face Soul Yoga pour que moi je sorte completement" (Session 2 — vision claire)
- "L'abonnement, c'est ma rente de base" (Session 2 — comprehension modele recurrent)
- "Inhabit your face, inhabit your life" (Session 2 — emergence promesse premium)
- "Il faut encore que je me legitimise" (Session 2 — syndrome de l'imposteur persiste)
- "Mon esprit est un peu fatigue" (Session 2 — signal surcharge)
- "Il faut qu'elle termine ce qui est commence avant de lancer autre chose" (Session 3 — Laurie sur Aurelia, eparpillement)
- "Je commence a avoir des regles assez strictes ou je ne travaille pas les week-ends" (Session 3 — Laurie, progression perso)
- "Il y a 50 messages par jour entre elles" (Session 3 — Laurie, volume communaute justifie mode mention-only)
- "À 17 €, les gens n'ont pas accès à moi. Mon énergie, je suis en train de la mettre sur Aurélia Del Sol." (Session 4 — séparation marque/personne actée)
- "C'est un recurring revenue qui est intéressant et qui peut être complètement scalable et qui, à terme, est revendable parce que la communauté est là, parce que les datas sont là" (Session 4 — vision business/actif)
- "Inconsciemment, c'est se dire je vais payer une formation, mais elle n'est pas assez chère" (Session 4 — déclic pricing via coach Romain)
- "Il faut qu'on arrive à prévoir un an à l'avance" (Session 4 — Laurie, cadre anti-chaos)
- "Il faut vraiment que le focus ce soit la méthode et pas toi" (Session 4 — Laurie, séparation marque/personne)
- "J'ai 130 000 personnes, mais je n'arrive pas a vendre. Soyez spectateurs, mais n'achetez pas." (Session 5 — syndrome imposteur qui sabote le business)
- "L'equipe qui t'amene a 5 chiffres, c'est pas celle qui t'amene a 6 ou 7." (Session 5 — lucidite sur le scaling)
- "J'ai l'impression d'avoir 1 000 tablettes ouvertes et d'etre perdue." (Session 5 — surcharge)
- "Face Soul Yoga, c'est un peu ma prison doree." (Session 5 — quand ton business t'enferme)
- "Je suis happee par des sujets qui me plaisent plus, mais j'ose pas y aller." (Session 5 — peur de lacher l'ancien)

## Pepites LinkedIn
> Extraites des sessions — reutilisables pour posts LinkedIn (anonymise : "une dirigeante" ou "une entrepreneuse")

| # | Citation / Situation | Angle LinkedIn | Hook potentiel | Utilise ? |
|---|---------------------|----------------|----------------|-----------|
| 1 | "Il faut qu'on sorte du truc de 'non mais on fait du bien, namaste'. Ca fait 6-7 ans que je fais ca" — prise de conscience de la valeur | Temoignage / Engagement | "Une entrepreneuse m'a dit : 'Il faut qu'on arrete de se cacher derriere le bien-etre. Ce qu'on fait a de la valeur.'" | Non |
| 1 | "2026, on arrete de travailler dans l'urgence" — engagement commun dirigeante + assistante | Coulisses / Expertise | "Premiere seance. Deux mots sur le tableau : STOP URGENCE. Pas un slogan. Un engagement." | Non |
| 1 | "J'ai du mal a me dire je vais partir une semaine et c'est OK" — la charge mentale de l'entrepreneuse solo | Empathie / Coulisses | "Combien de dirigeants peuvent partir une semaine sans que tout s'ecroule ? La plupart me repondent : zero." | Non |
| 1 | 80% du temps en operationnel, 50h/semaine, score process 2/5 — et pourtant CA 150K en croissance | Expertise (constat terrain) | "150 000 euros de CA. 50 heures par semaine. Et 80% du temps a eteindre des incendies." | Non |
| 1 | Clientes qui ecrivent sur WhatsApp le week-end sans limites — pas de cadre, pas de process | Expertise / Educatif | "Vos clients vous ecrivent le dimanche soir ? Le probleme, ce n'est pas eux. C'est qu'il n'y a pas de cadre." | Non |
| 2 | "Les gens me prennent pour l'epicerie" — demandes quotidiennes non cadrees qui devaluent l'expertise | Expertise / Educatif | "Une entrepreneuse m'a dit : 'Mes clientes me prennent pour l'epicerie.' Le vrai probleme ? Pas les clientes. L'absence de cadre." | Non |
| 2 | "17 EUR c'est le prix d'une creme Nivea" — recadrage valeur abonnement digital vs produit physique | Expertise / Educatif | "17 EUR par mois. 4 videos par semaine. C'est le prix d'une creme au supermarche. Pourquoi on a tant de mal a valoriser le digital ?" | Non |
| 2 | Passage de "je fais des lives toutes les semaines" a "ca tourne tout seul en evergreen" — transformation business model | Temoignage / Coulisses | "Elle faisait des lives chaque semaine. Epuisee. On a tout arrete. Et remplace par un systeme qui ne dort jamais." | Non |
| 2 | "Inhabit your face, inhabit your life" — emergence positionnement premium depuis le mass-market | Coulisses / Temoignage | "En 6 ans, elle est passee de 'touche ta ride' a 'rehabite ta vie'. Le positionnement premium ne se decrete pas. Il emerge." | Non |
| 2 | Architecture a 4 niveaux (17 EUR → 299-500 EUR → 3-6K EUR → 10-15K EUR) construite en 1 session | Expertise / Educatif | "4 niveaux de prix. Du 17 EUR au 15 000 EUR. Construits en 1 heure. La clarte d'offre, ca ne prend pas 6 mois." | Non |
| 3 | Laurie freine Aurelia — "il faut qu'elle termine avant de lancer autre chose" — le role de l'assistante garde-fou | Expertise / Coulisses | "Sa bras droit m'a dit : 'Il faut la ralentir.' Le meilleur conseil strat que j'ai entendu cette semaine." | Non |
| 3 | Revenue projection 634K EUR/an construit en 1 session — formation evergreen + accompagnements | Expertise / Educatif | "120 formations x 1 950 EUR. 1 000 accompagnements x 400 EUR. 634 000 EUR. Pas un reve. Un tableur." | Non |
| 3 | Le chatbot comme solution au "je ne travaille plus le week-end" — reminders automatiques dimanche soir | Temoignage / Coulisses | "Elle m'a dit : 'Les dimanches soirs, il faut envoyer un message aux membres.' Je lui ai repondu : 'Le bot le fera. Toi, tu restes en pyjama.'" | Non |
| 5 | "J'ai 130 000 personnes mais je n'arrive pas a vendre" — syndrome imposteur qui sabote le business | Expertise / Educatif | "130 000 abonnes. Et pas capable de vendre. Le syndrome de l'imposteur ne touche pas que les debutants." | Non |
| 5 | "L'equipe qui t'amene a 5 chiffres, c'est pas celle qui t'amene a 6 ou 7" — scaling et deuil des premieres equipes | Expertise / Coulisses | "La personne qui t'a aide a demarrer n'est pas toujours celle qui t'emmenera au niveau suivant. Et c'est OK." | Non |
| 5 | "Face Soul Yoga, c'est un peu ma prison doree" — quand ton propre business t'enferme | Temoignage / Empathie | "Elle a cree sa boite pour etre libre. 6 ans plus tard, c'est devenu sa prison doree." | Non |
| 5 | "Je suis happee par des sujets qui me plaisent plus, mais j'ose pas y aller" — peur de lacher l'ancien | Expertise / Educatif | "Vous savez deja ce que vous voulez faire. Vous n'osez juste pas lacher ce qui ne vous fait plus vibrer." | Non |

## Chatbot Telegram (@facesoulyoga_bot)

### Prestation
- **Inclus dans l'accompagnement** Clarte & Autonomie (agent IA sur-mesure, valeur 1 500 EUR au contrat)
- **Gratuit** pendant l'accompagnement (fev-aout 2026) — facturation a partir de la fin
- **Apres accompagnement** : 20 EUR/mois TTC (hebergement VPS + IA Claude + maintenance basique)
- **Evolutions majeures** : facturees en sus au cas par cas
- **Demarrage facturation** : apres validation par Laurie que le bot est operationnel

### Statut technique
- Deploye sur VPS Hostinger (n8n), 8 workflows actifs
- Commandes slash OK, ecoute passive IA OK
- **Decision 6 mars** : IA sur mention `@facesoulyoga_bot` uniquement (pas ecoute de tous les messages) — preserve les echanges entre membres + economise les tokens
- Reste a faire : fix escalade, charger FAQ, deployer sur vrai groupe
- **Reminders programmables** : dimanche soir pour lives du lundi (demande Laurie session 3)

## A faire (apres session 3)
- Tableau de bord KPIs par formation/offre (formation, abo, accompagnement) — priorite Catherine
- Envoyer CR session 3 a Laurie/Aurelia
- Planifier 1:1 Aurelia (mindset — demande Laurie)
- Attendre infos Laurie : FAQ, planning, groupe Telegram, chat_id
- Detail technique complet : voir `/Users/cath/.claude/projects/-Users-cath-Projects/memory/chatbot-fsy.md`

### Board Miro
- URL : https://miro.com/app/board/uXjVG2eDpcA=/
- 3 diagrammes : flux principal, enrichissement FAQ, planning hebdo
- Blocs rouges = donnees requises d'Aurelia

## Historique Sessions

| # | Date | Theme | Decision cle | Avancee notable |
|---|------|-------|-------------|-----------------|
| 1 | 2026-02-19 | Lancement strategique & cadrage priorites | Cartographier process (urgence), 2h CEO time/sem, pricing B2C 299+upsells→399, B2B evergreen, migration plateforme mi-mars, objectif 500K | Feuille de route presentee, templates SOP crees, Aurelia+Laurie alignees sur "stop urgence" et repositionnement premium |
| — | 2026-02-24 | Migration plateforme (travail inter-session) | Comparatif publie dans Notion : 3 architectures (Circle+Bunny, Kajabi seul, Kajabi+Bunny). Critere n°1 = portabilite. Kajabi deja en test. | Document aide a la decision pret pour vendredi 28/02. YouTube, Wistia, Wooskill, Skool elimines. Reco Catherine = Kajabi+Bunny (compromis simplicite/liberte) |
| 2 | 2026-03-02 | Migration Kajabi + Bunny, Architecture d'offre FSY & Pipeline Premium | Kajabi + Bunny.net confirmes, Circle elimine, Brevo maintenu. Architecture offre : FSY mass-market (abo 17 EUR + programmes 299-500 EUR) vs Aurelia Del Sol premium (Mastermind 3-6K EUR, 1:1 10-15K EUR). Fin lives hebdo, passage evergreen. Freelance Fiverr/ComeUp pour migration. Bot WhatsApp a explorer. | Decisions plateforme et offre actees. Laurie prend le lead migration. Antoine gere social media. Aurelia adopte Claude AI. KPIs : 6 700 membres totaux Uscreen, 613 courants, ~600 videos ~550 Go, communautes 238+184, ~300 abonnes app ~1 150-1 200 EUR |
| 3 | 2026-03-11 | Strategie offre FSY (evergreen + pricing) & Chatbot Telegram | Focus Master The Method → evergreen (mai 2026). Pricing stabilise : formation 1 950 EUR, abo 17 EUR/mois, accompagnement 399 EUR. YouScreen garde 1 an. Chatbot mode mention valide. | Laurie seule (Aurelia absente). Laurie freine Aurelia sur l'eparpillement. Revenue projection 634K EUR/an. Reminders programmables demandes pour chatbot. Automatisation contrats Kajabi a explorer. Session 1:1 Aurelia mindset a planifier. |
| 4 | 2026-03-16 | Repositionnement des marques & feuille de route opérationnelle | SESSION PIVOT : 3 entités distinctes actées (FSY Studio B2C, MTM B2B 2 900€, Aurélia Del Sol Premium). Migration Uscreen+Kajabi → Circle avant juillet. VA recrutée sur Upwork. Site FSY → éducatif. | Aurélia arrive avec décisions prises (coachs Mélanie + Romain). Prix MTM 1 950→2 900€. Laurie prend le lead Circle+migration. 4 temps forts/an FSY, 2 pushs/an MTM. Brevo = 13K contacts sous-exploités. Session Claude prévue. Feuille de route V2 construite. |
| 5 | 2026-03-24 | Repositionnement & organisation FSY (1:1 Aurelia) | Garder abo 17 EUR/mois mais Aurelia ne gere plus — process + freelance animation. Explorer packaging FSY comme actif revendable/franchise. Chatbot pret, en attente donnees. | Seance coaching 1:1 (sans Laurie). Laurie a dit "c'est pas moi le business manager". Syndrome imposteur profond (trauma exposition passe). Formation B2B a 2 900 EUR. Facture 2 750 EUR payée 26/03. |
| 6 | 2026-03-30 | Délégation, automatisation FSY et migration Circle + tunnel premium | Chatbot bascule WhatsApp (FSY) / Telegram (MTM). Laurie finit migrations puis transition. Programme Sommeil et Joie 397€ lancé 8 juin sous Aurélia Del Sol. Contact Studiolab pour site premium. | Aurélia claire sur séparation FSY/ADS. Laurie en fin de cycle (salaire doublé, livraison lente, bloque l'énergie). MTM sur Circle deadline 3 avril. FSY abo sur Circle deadline 31 juillet. Playbook montré avec process interactifs. Piste Lana (OBM). |
