...
def _get_notion_token():
    """Lire le token Notion depuis les settings Claude."""
    global _token_cache
    if _token_cache:
        return _token_cache
...
def _notion_request(method, url, body=None):
    """Requete HTTP vers l'API Notion."""
...
def _extract_text_from_blocks(blocks):
    """Extraire le texte lisible des blocs Notion."""
    lines = []
    for block in blocks:
...
def _extract_db_row(page):
    """Extraire les proprietes lisibles d'une ligne de database."""
...
def notion_query_database(database_id, filter=None, sorts=None):
    """Interroger une database Notion."""
